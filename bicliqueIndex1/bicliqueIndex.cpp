...
/**
 * 将 partialClique 按前缀树格式插入到 root 下。
 * root 可以是虚拟根（vertex = -1）或任何子树根。
 * 每个 partialClique[i] 表示从当前层到下一层的节点 vertex。
 */
void insertIntoPFTree(Node* root, const std::vector<int>& partialClique) {
    Node* current = root;
    // 对 partialClique 中的每个顶点依次插入
    for (int v : partialClique) {
        // 在 current 的子节点中查找 vertex == v
        Node* child = nullptr;
        for (int i = 0; i < current->child_count; ++i) {
            if (current->childs[i]->vertex == v) {
...
void addEdgeMaintain_Tree(Node *prefixTreeRoot) {
    std::string filePath = "D:\\school\\second year\\First Semester\\biclique index\\datasets\\uai_add.txt";
    std::vector<std::pair<int, int>> addEdge = readPairsFromFile(filePath);
    for (auto e : addEdge) {
        int v1 = e.first;
        int v2 = e.second;
...
void deleteEdgeMaintain_Tree(Node *prefixTreeRoot) {
    std::string filePath = "D:\\school\\second year\\First Semester\\biclique index\\datasets\\uai_delete.txt";
    std::vector<std::pair<int, int>> deleteEdges = readPairsFromFile(filePath);
    for (auto e : deleteEdges) {
        int v1 = e.first;
        int v2 = e.second;
